...
@Composable
@Preview
fun ViewPortfolioTabHistogram(modifier: Modifier = Modifier) {
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState())) {
            ...
            for (i in 0..portfolio.stocks.size - 1) {
                var currentTotalInvestedValue = portfolio.stocks[i].getCurrentTotalInvestedValue()
            ...
            for (i in 0..portfolio.stocks.size - 1) {
                ...
                Row(
                    ...
                    var currentStockTotalInvestedValue = portfolio.stocks[i].getCurrentTotalInvestedValue()
                    var currentStockTotalValue = portfolio.stocks[i].getCurrentTotalValue()
                    var currentPortfolioTotalInvestedValue = portfolio.getCurrentTotalInvestedValue()
                    ...
                    Row(Modifier.weight(0.4f).background(color = Color.White)) {
                        Text(
                            ...
                            text = portfolio.stocks[i].name,
                            fontWeight = FontWeight.Bold
                        )
                        var currentPercentOfPortfolio =
                            (currentStockTotalValue / currentPortfolioTotalValue * 100).toBigDecimal()
                                .setScale(2, RoundingMode.FLOOR).toDouble()
                        var originalPercentOfPortfolio =
                            (currentStockTotalInvestedValue / currentPortfolioTotalInvestedValue * 100).toBigDecimal()
                                .setScale(2, RoundingMode.FLOOR).toDouble()
                        var difference = (currentPercentOfPortfolio - originalPercentOfPortfolio).toBigDecimal()
                            .setScale(2, RoundingMode.FLOOR).toDouble()
                        var sign = if (difference >= 0.0) {
                            "+"
                        } else {
                            ""
                        }
                        Text(
                            modifier = Modifier.weight(0.1f).background(color = Color.White),
                            text = currentPercentOfPortfolio.toString() + "%"
                        )
                        Text(
                            modifier = Modifier.weight(0.25f).background(color = Color.White),
                            text = "(" + originalPercentOfPortfolio + sign + difference + "%)",
