...
fun ViewPortfolioTabHistogram(modifier: Modifier = Modifier) {
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        for(i in 0..portfolio.stocks.size-1) {
            ...
            Row(Modifier.fillMaxWidth().border(if(hoverState.value){
                ...
                Row(Modifier.weight(0.4f).background(color = Color.White)) {
                    ...
                    Text(
                        modifier = Modifier.weight(0.25f).background(color = Color.White), text = "("+originalPercentOfPortfolio+sign+difference + "%)", color=if(difference>=0.0){Color.Green}else{Color.Red}
                    )
                }
